// Perplexity Conversation Exporter Script
// This script exports the conversation from Perplexity as a markdown file
...
async function exportPerplexityConversation() {
  ...
  for (let i = 0; i < maxTurns; i++) {
    const turn = { query: '', answer: '' };
    if (queryEls[i]) turn.query = getQueryText(queryEls[i]);
    if (answerEls[i]) turn.answer = extractAnswerText(answerEls[i]);
  ...
  say([
    `✅  Done!`,
    `💬  ${qCount} prompt(s)  +  ${aCount} answer(s)`,
    `📁  1 file downloaded`,
  ].join('\n'), '#1a8a60');
