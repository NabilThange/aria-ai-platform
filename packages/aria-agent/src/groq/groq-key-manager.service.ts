...
@Injectable()
export class GroqKeyManagerService {
  private readonly logger = new Logger(GroqKeyManagerService.name);
  ...
  private loadKeys(): void {
    ...
    while (true) {
      const key = this.configService.get<string>(`GROQ_API_KEY_${keyIndex}`);
      if (!key) break;
    ...
    if (this.keys.length === 0) {
      this.logger.warn('No GROQ_API_KEY found. GroqService will not work properly.');
    } else {
      this.logger.log(`Loaded ${this.keys.length} Groq API key(s)`);
    }
  }
  ...
  getCurrentKey(): string | null {
    ...
    this.logger.error('All Groq API keys are disabled due to failures');
    return null;
  }
