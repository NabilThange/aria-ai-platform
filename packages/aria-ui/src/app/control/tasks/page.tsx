...
import { ControlTaskItem } from "@/components/tasks/ControlTaskItem";
...
function TasksPageContent() {
  ...
  const handleTabChange = (tab: TabKey) => {
    ...
    const newUrl = `/control/tasks${newSearchParams.toString() ? `?${newSearchParams.toString()}` : ""}`;
    ...
    <div className="flex h-screen flex-col bg-bytebot-bronze-light-1">
      ...
      <main className="hide-scrollbar flex-1 overflow-y-auto bg-bytebot-bronze-light-1 px-6 pt-6 pb-10">
        <div className="mx-auto max-w-4xl">
          {/* CONTROL MODE BADGE */}
          <div className="mb-4 bg-amber-500 text-white px-4 py-3 rounded-lg text-center font-semibold text-sm">
            🎮 CONTROL CENTER — Operator View
          </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className="space-y-3">
                {tasks.map((task) => (
                  <ControlTaskItem key={task.id} task={task} />
                ))}
              </div>
